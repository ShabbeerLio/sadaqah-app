...
const DonateCard = ({
  ...
  const progressPercent = Math.min(
    Math.round((i.amountReceived / i.totalPrice) * 100),
    100
  );
  console.log(i, "items");

  const tagColor = donationType === "following" ? "#800080" : "#00c5ff"; // purple : blue
  return (
    <div className="donate-card" key={i._id}>
      <div className="donate-header">
        <div className="donate-details">
          ...
          <p className="donation-description">
            {i.description.split(" ").slice(0, 10).join(" ")}
            {i.description.split(" ").length > 10 ? "..." : ""}
          </p>
        </div>
      ...
      <div className="donate-title">
        <img
          style={{ border: `2px solid ${tagColor}` }}
          img
          src={i?.institute.avatar}
          alt={i?.institute.userName}
        />
        <div className="donate-deatail">
          <div className="donation-name">
            <h5>
              {i?.institute.userName}
              <span className="verified">Verified</span>
            </h5>
            <p className="location">{i.institute.location}</p>
          </div>
        </div>
      </div>
      ...
      <div className="donate-pay">
        {user.role === "institute" ? (
          <div className="donate-paybtn">
            <p onClick={() => handleDonet("list", i._id)}>View All</p>
            <p>
              <Link
                to={"/request-details"}
                state={{ item: i._id }}
